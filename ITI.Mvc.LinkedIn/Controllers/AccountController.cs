...
namespace ITI.Mvc.LinkedIn.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                //UserName = string.Format("{0} {1}", model.Fname, model.Lname)
                var user = new ApplicationUser {  Email = model.Email, Fname = model.Fname, Lname = model.Lname , UserName = model.Email};
        ...
        public ActionResult Search(SearchViewModel myview)
        {
            string username = myview.Username.ToUpper();
            List<ApplicationUser> searchedapplicationusers = new List<ApplicationUser>();
            List<ApplicationUser> allapplicationusers = new List<ApplicationUser>();
            allapplicationusers = UserManager.Users.ToList();
            string s;
            for (int i = 0; i < allapplicationusers.Count; i++)
            {
                s = (allapplicationusers[i].Fname + " " + allapplicationusers[i].Lname).ToUpper();
                if ((allapplicationusers[i].Fname).ToUpper() == username || (allapplicationusers[i].Lname).ToUpper() == username || s == username || (allapplicationusers[i].Fname).ToUpper().Contains(username)|| (allapplicationusers[i].Lname).ToUpper().Contains(username))
                {
                    searchedapplicationusers.Add(allapplicationusers[i]);
                }
            ...
            return PartialView("_PartialSearchForUsers", searchedapplicationusers);
        }
